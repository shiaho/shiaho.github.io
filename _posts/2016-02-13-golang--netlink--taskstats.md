...
刚开始了解 taskstate 的目的是为了给项目hickwall的agent增加获取io的top进程，这个信息可以通过读取proc里面的io来获取，但是考虑到要读取所有进程proc文件同样很大的io消耗，有在centos下有一个 `iotop` 的命令，查看其源码 发现是使用netlink来获取的，于是决定采用netlink来获取。
